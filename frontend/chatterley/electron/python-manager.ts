...
export class PythonServerManager {
  ...
  private getOumiRootPath(): string {
    ...
    // Legacy fallback when frontend lived inside oumi/
    ...
    const errorMessage = `Unable to determine Oumi root. Looked for backend/oumi under ${repoRoot}`;
    log.error(`[getOumiRootPath] ${errorMessage}`);
  ...
  public async testModel(configPath: string): Promise<{ success: boolean; message?: string }> {
    ...
    try {
      ...
      const result = await new Promise<{ success: boolean; message?: string }>(async (resolve) => {
        ...
        const cleanup = (terminate: boolean = false) => {
          if (networkMonitorInterval) {
            clearInterval(networkMonitorInterval);
            networkMonitorInterval = null;
          }
          if (
            ...
            try {
              ...
              log.warn('Failed to terminate test process during cleanup:', killError);
            }
          }
        };
