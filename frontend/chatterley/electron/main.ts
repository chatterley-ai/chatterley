...
import { app, BrowserWindow, Menu, ipcMain, dialog, shell, session, screen } from 'electron';
...
function createMainStore(): Store {
  const opts: Store.Options<any> = {
    ...
    defaults: {
      windowBounds: getDefaultWindowBounds(),
      pythonPort: 9000,
      lastSession: 'default'
    },
...
class ChatterleyApp {
  ...
  private createMainWindow(): void {
    const bounds = this.calculateLaunchBounds();
  ...
  private calculateLaunchBounds(): WindowBounds {
    try {
      const primaryDisplay = screen.getPrimaryDisplay();
      const workArea = primaryDisplay.workAreaSize ?? primaryDisplay.size;
      const targetWidth = Math.round(workArea.width * (2 / 3));
      const targetHeight = Math.round(workArea.height * (2 / 3));
      ...
      log.warn('[Main] Failed to calculate launch bounds from display info:', error);
