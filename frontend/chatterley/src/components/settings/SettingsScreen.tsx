    ...
    if (apiClient.isElectronApp()) {
      ...
      if (backendSystemInfo) {
        ...
        return {
          ...
          platform: (backendSystemInfo.platform as string) || (browserInfo as { platform?: string }).platform || 'Unknown',
          architecture: (backendSystemInfo.architecture as string) || 'Unknown',
          totalRAM: (backendSystemInfo.totalRAM as number) || browserInfo.estimatedMemoryGB || 0,
          cudaAvailable: (backendSystemInfo.cudaAvailable as boolean) || false,
          cudaDevices: (backendSystemInfo.cudaDevices as Array<{vram: number}>) || ([] as Array<{vram: number}>),
        };
      }
    }
      ...
      <div className="bg-card border rounded-lg p-4 space-y-4">
        ...
        <label className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Downscale image attachments to reduce upload size (default 640×360)
            </div>
          </div>
          <input
            type="checkbox"
            checked={mediaSettings.resizeImages}
            onChange={(e) => handleMediaSettingsChange({ resizeImages: e.target.checked })}
            className="rounded"
          />
        </label>

        {mediaSettings.resizeImages && (
          <div className="pl-6 grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                min={64}
                step={1}
                value={mediaSettings.targetImageWidth}
                onChange={(e) => {
                  const parsed = parseInt(e.target.value, 10);
                  handleMediaSettingsChange({
                    targetImageWidth: Number.isFinite(parsed) && parsed > 0 ? parsed : mediaSettings.targetImageWidth,
                  });
                }}
                className="w-full px-3 py-2 bg-background border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary"
              />
            ...
            <div>
              ...
              <input
                ...
                min={64}
                step={1}
                value={mediaSettings.targetImageHeight}
                onChange={(e) => {
                  const parsed = parseInt(e.target.value, 10);
                  handleMediaSettingsChange({
                    targetImageHeight: Number.isFinite(parsed) && parsed > 0 ? parsed : mediaSettings.targetImageHeight,
                  });
                }}
                className="w-full px-3 py-2 bg-background border rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary"
              />
            ...
            <p className="col-span-2 text-xs text-muted-foreground">
              Images larger than these dimensions are proportionally scaled before they are sent to the model.
            </p>
          </div>
        )}
