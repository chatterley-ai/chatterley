...
class WebChatSession:
    ...
    def __init__(self, session_id: str, config: InferenceConfig):
        ...
        self.inference_engine_error: Optional[str] = None
    ...
    def _build_inference_engine(self, config: InferenceConfig):
        ...
        try:
            ...
            self.inference_engine_error = error_message
            logger.exception(
                "Failed to initialize inference engine for session %s (engine=%s, model=%s)",
                self.session_id,
                getattr(config.engine, "name", config.engine) or InferenceEngineType.NATIVE,
                getattr(config.model, "model_name", "unknown"),
            )
            raise RuntimeError(
                f"Failed to initialize inference engine for session {self.session_id}: {error_message}"
            ) from e
