...
def resolve_frontend_dir(oumi_root_dir: str) -> Optional[str]:
    """Locate the frontend directory for the WebChat UI.

    Checks the new chatterley layout (repo/frontend/chatterley), legacy layout
    (backend-root/frontend), and optional environment overrides.
    """

    candidates: list[str] = []

    override = os.getenv("OUMI_FRONTEND_DIR")
    if override:
        candidates.append(override)

    # New layout: backend/oumi lives alongside frontend/chatterley at repo root
    repo_root = os.path.abspath(os.path.join(oumi_root_dir, "..", ".."))
    candidates.append(os.path.join(repo_root, "frontend", "chatterley"))

    # Handle case where backend is checked out without parent chatterley repo
    candidates.append(
        os.path.abspath(os.path.join(oumi_root_dir, "..", "frontend", "chatterley"))
    )

    # Legacy layout where frontend lived within the oumi repository
    candidates.append(os.path.join(oumi_root_dir, "frontend"))
...
def webchat(
    ...
    frontend_dir = resolve_frontend_dir(oumi_root_dir)

    if not frontend_dir:
        logger.error("❌ Frontend directory not found. Tried new chatterley layout and legacy oumi/frontend path.")
        logger.info(
            "💡 Set OUMI_FRONTEND_DIR to the frontend path or clone chatterley so frontend/chatterley exists."
        )
        raise typer.Exit(1)
